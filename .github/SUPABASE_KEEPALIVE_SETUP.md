...
4. Add two secrets:
  ...
  - **Name**: `REACT_APP_SUPABASE_PUBLISHABLE_DEFAULT_KEY` | **Value**: Your Supabase publishable/anon key. Do not use a secret or service-role key.
...
2. Verify your `REACT_APP_SUPABASE_PUBLISHABLE_DEFAULT_KEY` is the publishable/anon key, not a secret or service-role key
3. Check the workflow logs in the **Actions** tab for error details
4. Ensure the values are named exactly `REACT_APP_SUPABASE_URL` and `REACT_APP_SUPABASE_PUBLISHABLE_DEFAULT_KEY`
