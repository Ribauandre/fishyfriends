...
- **Looks and Marina's Outfitters** (`utils/anglerLook.js`, `look`/`wardrobe` on `game_profiles` — migration `0022`): the angler is the ChatGPT-drawn sprite sheet `art/angler-sheet.png` — **bald, clean-shaven**, pale shirt, olive waders — sliced into the strips in `assets/angler` by `scripts/anglerSlice.mjs`, which does reproduce them from that sheet: it thresholds the alpha haze, drops the navy label chips, strips the drawn fishing line and its lure (the game draws its own line from the rod tip, and the sheet's wander off across three frames), and takes each pose as one connected component — a rod arcing over the next frame is still its caster's, because it is joined to his hands and to nothing else. The rows are `idle` 5, `walk` 5 (sliced, not kept — there is no walk), `cast` 5, `reel` 6, `celebrate` 4 and `hurt` 3; cast's fourth frame is drawn without a rod and a head shorter than the row, so `KEEP` leaves it out and puts the other four in the order a cast actually goes, because the game holds the last one for as long as the line is out. There is no `fishon` row any more: the hookset is the reel strip's first beats and a lost fish gets the sheet's own slump. Frames land in the box the stage has always used — 250×160, feet at (80, 154) — which leaves the ten rows of headroom above the crown that a painted hat needs; `strips.json` records it. Each strip has a part mask in `assets/angler/masks` (red channel = part id: skin, shirt, waders, boots, rod, line work) and `utils/anglerAnchors.json` holds each frame's head box, face box and facing — from `scripts/anglerMasks.mjs`, whose previews are in `masks/*.preview.png`. That script leans on shape where colour cannot decide: the rod is what vanishes under an erosion, the keyline is the outermost pixel of the body, the waders' near-black folds are told from line work by their olive cast, and the head box is cut at the chin, where the widest row of the skin patch has lost most of its width to the neck. The sheet only ever draws the angler in a three-quarter turn to his right, so `FACING` is one stated fact about the art rather than something guessed at per frame. The character is dressed at runtime by `utils/anglerPaint.js`. Parts that keep their shape are dyed (`tintPixel`, luminance-preserving — skin tone, shirt, waders, boots, rod). The head is **built on** rather than sculpted: the sheet gives a bare skull and a bare jaw, so hair is laid over the top of the skull following the silhouette the art drew, a beard is painted onto the jaw, and a hat is raised over the crown into the headroom. That is the whole reason this sheet replaced the one before it, which was drawn in a cap and a full beard that every look had to fight with. Landmarks are fractions of the head box (`EYES`, `MOUTH`, `BRIM` per hat kind) and `frontness` says how far forward a column is, so a hairline runs deep at the back and shallow at the forehead, and a beard rides high at the sideburn and drops to the mouth at the chin. Everything is shaded like a dome (`domeShade`) and given the art's line back (`OUTLINE`); a cap gets a peak out past the face, a beanie a band and a pompom, a bucket, straw or cowboy hat a brim, a visor just a band and a peak over whatever the hairstyle leaves on the crown; long hair is a tapered fill of the open pixels down the back of the neck. `paletteFor(look)` is the pure look → dye targets + `head` plan the painter and its tests share; `DEFAULT_LOOK` is the art exactly as drawn — bald, clean-shaven, bare-headed — so the stock strips can show unpainted, and where there's no canvas (jsdom) they show anyway with previews carrying `data-painted="no"`. Skin tone, hair style/colour and facial hair are free; hats, shirts, rod colours, boots and waders are bought once (`purchaseApparel`) and worn via `saveLook`, which re-validates against what's owned — the wardrobe keys are stable, so a swap of the base art never orphans a purchase. `AnglerPreview` frames `STILL_WINDOW` on the strip or the painted still so both come out the same size. A new hat is a `kind` the painter knows how to build plus its colours in `WARDROBE`, not a drawing; a new base sheet from the ChatGPT app (the API's reproductions of it were not usable) means a new run through the slicer and mask tool, and both scripts need Playwright with Chromium. Presence carries `look` so the crew wear theirs.
