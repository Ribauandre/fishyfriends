...
- **Looks and Marina's Outfitters** (`utils/anglerLook.js`, `look`/`wardrobe` on `game_profiles` — migration `0022`): the angler is a ChatGPT-drawn sprite sheet — `art/angler-sheet.png`, a bald, clean-shaven, bare-headed man in blue jeans and an olive vest over a blue shirt, on a transparent background — cut into the strips in `assets/angler` by `scripts/anglerSlice.mjs`, which finds the rows in the alpha row profile and each frame as one connected component of a row (nothing on that sheet is on a grid, and a rod arcing over the next figure's head has to stay with its own frame); `strips.json` beside the strips records the frame box and each strip's frame count, and `utils/anglerSprites.js` reads it so the two never drift. The character is dressed at runtime by `utils/anglerPaint.js`. Each strip has a part mask in `assets/angler/masks` (red channel = part id: skin, shirt, vest, trousers, boots, rod) and `utils/anglerAnchors.json` holds each frame's head box — both built from the art by `scripts/anglerMasks.mjs` (re-run it if the strips change; it also writes `masks/*.preview.png` to eyeball). The rod is found there by shape rather than colour — the long thin thing outside the body, the body being the silhouette eroded until nothing as narrow as a rod survives — then grown back along its own brown into the fist, which is what keeps it apart from the boots it is painted almost the same colour as; the line it throws is thin too but bright blue, and is left in no part so no dye reaches it. Because the art wears nothing, a look only ever adds: masked parts are dyed (`paintPixels`, pure, luminance-preserving: skin tone, vest, trousers, boots, rod) and drawings from `assets/angler/parts` are composited on the head at the anchors (`placeOn`/`PLACEMENTS`, all against the head box) — hair, then facial hair, then a hat over both, with long hair drawn behind the body. Stubble is the one exception: a checker of hair colour mixed lightly into the skin of `jawBox`. `paletteFor(look)` is the pure look → paint plan, and `DEFAULT_LOOK` is the art exactly as drawn (bald, clean-shaven, bare-headed, stock colours) so the stock strips can show unpainted. Skin tone, hair style/colour and facial hair are free; hats, vests, trousers, boots and rod colours are bought once (`purchaseApparel`) and worn via `saveLook`, which re-validates against what's owned — a look saved against an older wardrobe just falls back. Where there's no canvas (jsdom) the stock strips show and previews carry `data-painted="no"`. `AnglerPreview` frames `STILL_WINDOW` on the strip or the painted still with background-size/position maths so both come out the same size, since the frame box is much wider than the figure. New hats, hair or beards are generated as standalone transparent sprites (side view, facing right, `scripts/generateImage.mjs --transparent`), processed like any prop, and listed in `WARDROBE`/`HAIR_STYLES`/`BEARD_STYLES` with a `PLACEMENTS` rule; the base sheet came from the ChatGPT app (the API's reproductions of it were not usable), so a new base sheet means a new upload run through the slicer and mask tool, not an API call. Presence carries `look` so the crew wear theirs.
