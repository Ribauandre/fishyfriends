...
- **Looks and Marina's Outfitters** (`utils/anglerLook.js`, `look`/`wardrobe` on `game_profiles` — migration `0022`): the angler is one drawing recoloured per look. Each strip in `assets/angler` has a part mask in `assets/angler/masks` (red channel = part id; generated offline by classifying the art) and `utils/anglerAnchors.json` holds each frame's hat and beard boxes; `paintPixels` is the pure dye (tests run it on plain arrays) and `utils/anglerPaint.js` does it on a canvas at runtime, draws overlay hats (`assets/angler/hats`) at the anchors and caches data URLs by `lookKey`. Where there's no canvas (jsdom) it resolves null and the stock art shows. Skin, facial hair and hair colour are free; hats, rod colours, boots and waders are bought once (`purchaseApparel`) and worn via `saveLook`, which re-validates against what's owned. Presence carries `look` so the crew wear theirs.
