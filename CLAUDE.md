...
- **Looks and Marina's Outfitters** (`utils/anglerLook.js`, `look`/`wardrobe` on `game_profiles` — migration `0022`): the angler is the ChatGPT-drawn sprite sheet (the strips in `assets/angler`, sliced from the sheet by `scripts/anglerSlice.mjs`; `strips.json` beside them records the frame box), dressed at runtime by `utils/anglerPaint.js`. Each strip has a part mask in `assets/angler/masks` (red channel = part id: skin, beard, cap, cap panel, jacket, waders, boots, rod) and `utils/anglerAnchors.json` holds each frame's cap, beard and face boxes — both built from the art by `scripts/anglerMasks.mjs` (re-run it if the strips change; it also writes `masks/*.preview.png` to eyeball). A look is applied by dyeing masked parts (`paintPixels`, pure, luminance-preserving: skin tone, the beard to the hair colour or into jaw, the cap to a cap colour or into hair, waders, boots, rod) and by compositing drawings from `assets/angler/parts` on the head at the anchors (`placeOn`/`PLACEMENTS`): the non-cap hats, hair when the cap is off, and facial hair other than the full beard the art already has (stubble is a dither). `paletteFor(look)` is the pure look → paint plan. Skin tone, hair style/colour and facial hair are free; hats, rod colours, boots and waders are bought once (`purchaseApparel`) and worn via `saveLook`, which re-validates against what's owned. Where there's no canvas (jsdom) the stock strips show and previews carry `data-painted="no"`. New hats, hair or beards are generated as standalone transparent sprites (side view, facing right, `scripts/generateImage.mjs --transparent`), processed like any prop, and listed in `WARDROBE`/`HAIR_STYLES`/`BEARD_STYLES` with a `PLACEMENTS` rule; the base strips themselves came from the ChatGPT app (the API's reproductions of the sheet were not usable), so a new base sheet means a new upload run through the slicer and mask tool, not an API call. Presence carries `look` so the crew wear theirs.
