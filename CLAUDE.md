...
- **Looks and Marina's Outfitters** (`utils/anglerLook.js`, `look`/`wardrobe` on `game_profiles` — migration `0022`): the angler is a *rigged, code-drawn* pixel sprite, not a painting. `utils/anglerRig.js` is the skeleton — a 144×120 native-pixel frame with the feet anchored, a keyframe list per action (idle/cast/reel/fishon/celebrate, counts in `FRAME_COUNTS`), a two-bone arm solver and the rod geometry (`rodTipFor` is where the line and pennant attach) — and `utils/anglerDraw.js` renders each solved frame part by part (back arm, pack/long hair, legs, torso, head, facial hair, hat, rod, front arm), snapping and outlining every layer, then scales the frames 3× with no smoothing into the strips the stage animates (`renderAngler(look)` → data URLs cached by `lookKey`; `renderStill` for previews and the rack thumbnails). `paletteFor(look)` is the pure look → colours step. Skin tone, hair style/colour and facial hair (`SKIN_TONES`, `HAIR_STYLES`, `HAIR_COLORS`, `BEARD_STYLES`) are free; hats (each a drawn `style`), rod colours, boots and waders are bought once (`purchaseApparel`) and worn via `saveLook`, which re-validates against what's owned. The PNGs in `assets/angler` are this renderer's output for the default look — the fallback where there's no canvas (jsdom shows them and `data-painted="no"`) — and are regenerated with `node scripts/renderAnglerSheets.mjs` (needs Playwright + Chromium) whenever the rig or drawing changes; never hand-edit or AI-generate them. Add a cosmetic by drawing it in `anglerDraw.js` and listing it in `WARDROBE`, not by adding art files. Presence carries `look` so the crew wear theirs.
