...
function PersonalBestCard({ best, onRemove }) {
  return <div className="best-card">
    <button type="button" className="best-card-remove" onClick={() => onRemove(best.id)} aria-label={`Remove ${best.species} personal best`}>×</button>
...
function PersonalBestForm({ onSave }) {
  ...
  async function submit(event) {
    ...
    else { setSpecies(''); setSizeLabel(''); setCaughtAt(''); setFile(null); event.target.reset(); }
  }
  ...
  return <form className="best-form" onSubmit={submit}>
    ...
    <label>Date<input type="date" value={caughtAt} onChange={(e) => setCaughtAt(e.target.value)} /></label>
