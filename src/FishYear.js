...
export default function FishYear() {
  ...
  const [form, setForm] = useState({ species: '', date: new Date().toISOString().slice(0, 10), photoFile: null });
  ...
  async function logCatch(event) {
    ...
    setShowForm(false);
    setForm({ species: '', date: new Date().toISOString().slice(0, 10), photoFile: null });
  }
