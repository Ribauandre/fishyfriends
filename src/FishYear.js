...
export default function FishYear() {
  ...
  const monthsCovered = new Set(catches.map((c) => c.month)).size;
  const anglersOnBoard = new Set(catches.map((c) => c.user_id)).size;
  const topSpecies = (() => {
