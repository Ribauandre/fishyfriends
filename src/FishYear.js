...
export default function FishYear() {
  ...
  return <main className="content-shell challenge-page"><div className="page-intro challenge-intro"><div><span className="eyebrow">2026 · MONTHLY CHALLENGE</span><h1>Fish Year</h1><p>One fish, every month. Keep the streak alive with the crew.</p></div><div className="challenge-actions"><button className="button button-primary" type="button" onClick={() => setShowForm(true)}>Log a catch <span>＋</span></button><Link className="button button-quiet" to="/profile">My profile <span>→</span></Link></div></div><section className="personal-year-board"><div className="section-heading"><div><span className="eyebrow">YOUR 2026</span><h2>Catch the whole year</h2></div><div className="year-legend"><span><i className="legend-caught" />Caught</span><span><i className="legend-missing" />Not caught</span></div></div><div className="year-month-grid">{yearMonths.map((month) => { const caught = caughtMonths.includes(month); return <div className={`year-month ${caught ? 'is-caught' : 'is-missing'}`} key={month}><span className="year-month-number">{String(yearMonths.indexOf(month) + 1).padStart(2, '0')}</span><strong>{month.slice(0, 3)}</strong><span className="year-month-status">{caught ? '✓ Caught' : '— Not caught'}</span></div>; })}</div><div className="year-board-footer"><strong>{caughtMonths.length} of 12 months caught</strong><span>{12 - caughtMonths.length} months still open</span></div></section><div className="challenge-summary"><div><strong>02</strong><span>months logged</span></div><div><strong>08</strong><span>anglers in</span></div><div><strong>10</strong><span>months to go</span></div></div><section className="table-card"><div className="section-heading"><div><span className="eyebrow">THE BOARD</span><h2>Monthly progress</h2></div><span className="status-badge status-badge-muted">LIVE</span></div><Participants newEntry={newEntry} /></section><section className="rules-strip"><span className="rule-index">RULES / 01</span><p>Catch at least one fish a month and share a photo with the crew. Saltwater, freshwater, big or small, it all counts.</p></section>{showForm && <div className="catch-modal-backdrop" role="presentation" onClick={() => setShowForm(false)}><form className="catch-modal" onSubmit={logCatch} onClick={(event) => event.stopPropagation()}><div className="section-heading"><div><span className="eyebrow">NEW CATCH</span><h2>Log your fish</h2></div><button className="modal-close" type="button" onClick={() => setShowForm(false)} aria-label="Close log catch form">×</button></div><label>Month<select value={form.month} onChange={(event) => setForm({ ...form, month: event.target.value })}>{yearMonths.map((month) => <option key={month}>{month}</option>)}</select></label><label>Species<input required value={form.species} onChange={(event) => setForm({ ...form, species: event.target.value })} placeholder="What did you catch?" /></label><label>Date<input required type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /></label><label>Photo<input type="file" accept="image/*" onChange={handlePhoto} /></label><div className="form-actions"><button className="button button-primary" type="submit">Add to Fish Year <span>→</span></button></div></form></div>}</main>;
}
