-- Adds Fish Year catch persistence to a project that already has profiles, avatars, and
-- personal_bests deployed (running the full schema.sql there fails on its first
-- `create table` — see migration-personal-bests.sql for that same situation). This file
-- is safe to run more than once — the table uses IF NOT EXISTS and policies are dropped
